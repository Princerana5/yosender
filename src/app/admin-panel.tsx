              ...
              <tbody>
                {users.map((u:any) => (
                  <tr key={u.id} className="border-t border-slate-100 hover:bg-slate-50/60">
                    ...
                    <td className="px-3 py-3"><div className="flex flex-col gap-1 items-center"><span className={`text-[10px] font-bold tracking-widest px-2 py-1 rounded-full border ${u.isBanned ? "bg-red-50 text-red-700 border-red-200" : "bg-emerald-50 text-emerald-700 border-emerald-200"}`}>{u.isBanned ? "BANNED" : "ACTIVE"}</span><span className="text-[10px] font-semibold text-slate-500" title={u.emailVerified === false ? "Signed up but never verified the email OTP" : "Email verified"}>{u.authMethod}{u.emailVerified === false ? " · unverified" : ""}</span></div></td>
