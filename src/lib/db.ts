...
function ensure(){ if(!fs.existsSync(DB_DIR)) fs.mkdirSync(DB_DIR,{recursive:true}); for(const f of [USERS_FILE,ACCS_FILE,CAMPS_FILE,RENTAL_POOL_FILE,RENTALS_FILE,TEMPLATES_FILE]) if(!fs.existsSync(f)) fs.writeFileSync(f,"[]"); }
...
export type Template={id:string;userId:string;name:string;message:string;category:string;image?:string|null;createdAt:string};
export function getTemplates():Template[]{return read<Template>(TEMPLATES_FILE);}
export function saveTemplates(t:Template[]){write(TEMPLATES_FILE,t);}
